...
import time
import random
from modeles import snake
from modeles import snake_eat
from modeles import snake_add
...
class python_snake:  # Двигать тело змеюки в текущую сторону на 1 шаг
    ...
    def create_head_food(self):
        ...
        if rand_vect == 1:
            ...
            self.__vector = self.CONST.UP.value
        self.head = snake_add.element_square(self, self.__snake_x,
                                        self.__snake_y,
    ...
    def start(self):  # Бесконечный цикл движения змейки
        ...
        while i == 0:
            self.step(add)
            if snake_eat.eat(self) == 1:
                add = 'add'
                self.speed('+')
            elif add == 'add':
                add = 'del'
            if snake.bump_wall(self) == 'the end':
                break
            if snake.bump_body(self) == 'the end':
                break
    ...
    def step(self, add):  # Двигать тело змеюки в текущую сторону на 1 шаг
        ...
        self.head.x += deltax
        self.head.y += deltay
        self.head = snake_add.element_square(self, self.head.x, self.head.y,
                                        self.CONST.SNAKE_THICKNESS.value,
                                        self.CONST.SNAKE_HCOLOR.value)
    ...
    class food:
        def add(self):
            ...
            self.food.y = random.randint(self.CONST.FOOD_THICKNESS.value
                                         // 2, self.canv_height
                                         - self.CONST.FOOD_THICKNESS.value // 2)
            self.food.body = snake_add.element_square(self, self.food.x,
                                                 self.food.y,
...
def main():
    ...
    root = Tk()
    root.title('Гра Змійка :)')
    root.geometry('800x600+150+150')

    frame = Frame(root, width=740, height=90, bg='#f2ffe0')
    frame.place(x=30, y=5)
    text = Label(root, text='''Командна робота КНІТ-23. Студентів: Коніщука Олександра,Галух Анни,Гауросіка Ілльоі,Гончарука Антона   ''',
                 bg='#f2ffe0', width=100)
